...
public class Main {
    ...
    //ДІЯ 2: Пишемо реалізацію конструктора для класа Main, в якому описуємо графічний інтерфейс користувача та обробники подій
    public Main() {

        //Створюємо і налаштувємо вікно програми (JFrame)
        JFrame MainForm = new JFrame("Обмін валют"); // Створюємо новий екземпляр класа з ідентифікатором MainForm і заголовком "Обмін валют"
        ...
        JTextField DollarsRate = new JTextField(); // Створюємо компонент для відображення і редагування текстової строки (екземпляр класу JTextField), в якому буде вноситися курс долара
        ...
        JTextField HryvnaSum = new JTextField(); // Створюємо компонент для відображення текстової строки (екземпляр класу JTextField), в якому буде відображатися перерахована сума в гривнах
        HryvnaSum.setBounds(10,130,270,30); // Визначаємо його положення на формі і розміри
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                new Main(); // В методі run() створюэться конструктор для класа Main, в якому буде опис інтерфейса і обробників подій
            }
        });
    }
}
